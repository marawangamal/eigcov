...
class Batcher(object):
    """Returns batches of data, with optional distributed-sampler support."""
    ...
    def get_splitOfBatches(self, split, template_idx, is_evaluation):
        assert split.lower() in [
            "validation", "validation_full", "train", "test",
        ], f"Evaluation Split {split} not defined"

        dataset = self.dataset_reader.get_dataset(split, template_idx, is_evaluation)
        pytorch_dataset = self.createPytorchDataset_fn(dataset)
