...
def _topk_values_mask(M, K=0.7, return_mask=False):
    if K > 1:
        K /= 100
...
def _resolve_zero_signs(sign_to_mult, method="majority"):
...
def _resolve_sign(Tensor):
    sign_to_mult = torch.sign(Tensor.sum(dim=0))
    sign_to_mult = _resolve_zero_signs(sign_to_mult, "majority")
    return sign_to_mult


def _disjoint_merge(Tensor, merge_func, sign_to_mult):
...
def ties_merging(
    ...
    updated_checks, *_ = _topk_values_mask(
        all_checks, K=reset_thresh, return_mask=False
    )
    print(f"RESOLVING SIGN")
    final_signs = _resolve_sign(updated_checks)
    assert final_signs is not None

    print(f"Disjoint AGGREGATION: {merge_func}")
    merged_tv = _disjoint_merge(updated_checks, merge_func, final_signs)
...
def merge_ties(taus, merge_func="", **kwargs):
